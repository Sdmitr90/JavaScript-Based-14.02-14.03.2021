document.addEventListener('DOMContentLoaded', () => {
    ...
    var FIGURES = [
        ...
        {
            cells: [2, BOARDWIDTH + 2, BOARDWIDTH * 2 + 2, BOARDWIDTH * 3 + 2],
            width: 4,
            color: 'iFigure',
        },
        {   // новая фигура
            ...
            width: 2,
            color: 'LFigure', // _| фигура
        },
        {   // новая фигура
            cells: [0],
            width: 1,
            color: 'dotFigure', // . фигура
        },
        {// новая фигура
            cells: [1, BOARDWIDTH + 1],
            width: 2,
            color: 'iiFigure', // -- фигура
        },
        {   // новая фигура
            cells: [1, BOARDWIDTH * 2 + 1, BOARDWIDTH + 1],
            width: 3,
            color: 'llFigure', // --- фигура
        },
        {   // новая фигура
            cells: [1, 2, BOARDWIDTH + 1, BOARDWIDTH * 2 + 1, BOARDWIDTH * 2 + 2],
            width: 3,
            color: 'LIFigure', // |__| фигура
        }
    ]
    ...
    var nextElement = document.getElementById('nextEl');
    ...
    var openBoardn = false;
    function oBoard() {
        if (gamePaused && openBoardn == false) {
            gameBoard.cells.forEach((cell) => cell.element.className = cell.haveBlock ? cell.color : 'closed');
            drawFigure();
            openBoardn = true;
        } else if (gamePaused && openBoardn == true) {
            gameBoard.cells.forEach((cell) => cell.element.className = 'closed');
            openBoardn = false;
        }
    }
    ...
    function setNewFigure() {
        ...
        // Отображаю следующий элемент
        nextFigure = gameBoard.currentFigureNumber;
        var table = document.createElement("table");
        function nextElementName() {
            for (var i = 0; i <= FIGURES[nextFigure].cells.length-1; i++) {
                var tr = document.createElement('tr');    
                for (var j = 0; j <= FIGURES[nextFigure].cells.length-1; j++) {
                    var td = document.createElement('td');
                    for (x = 0; x < 10; x++) {              // рисую элемент на каждой строке ( до 4х строк)
                        if (FIGURES[nextFigure].cells[x] == j  && i == 0) {
                            td.className = FIGURES[nextFigure].color;
                        }
                        if (((FIGURES[nextFigure].cells[x] - 10) == j) && i == 1){
                            td.className = FIGURES[nextFigure].color;
                        }
                        if (((FIGURES[nextFigure].cells[x] - 20) == j) && i == 2){
                            td.className = FIGURES[nextFigure].color;
                        }
                        if (((FIGURES[nextFigure].cells[x] - 30) == j) && i == 3){
                            td.className = FIGURES[nextFigure].color;
                        }
                    }
                    tr.appendChild(td);
            ...
            document.body.lastChild.replaceWith(table);
        }
        nextElementName();
    }
    ...
    function initBoard() {
        ...
        openBoard.addEventListener('click', oBoard);
